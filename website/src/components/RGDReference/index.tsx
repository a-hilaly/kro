...
export default function RGDReference(): JSX.Element {
  ...
  useEffect(() => {
    fetch('/crds/kro.run_resourcegraphdefinitions.yaml')
      .then((response) => response.text())
      .then((text) => {
        const crdData = yaml.load(text);
        const { specFields: parsedSpecFields, statusFields: parsedStatusFields } = parseFieldsFromCRD(crdData);
        setSpecFields(parsedSpecFields);
        setStatusFields(parsedStatusFields);
        setLoading(false);
      })
      .catch((err) => {
        setError(`Failed to load CRD: ${err.message}`);
        setLoading(false);
      });
  }, []);

  if (loading) {
    return <div>Loading API reference...</div>;
  }
