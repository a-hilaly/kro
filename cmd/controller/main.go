...
func main() {
	var (
		...
		resyncPeriod                   int
		queueMaxRetries                int
		instanceDefaultRequeueDuration time.Duration
	...
	flag.DurationVar(&instanceDefaultRequeueDuration, "instance-controller-default-requeue-duration", 3*time.Second,
		"default duration to wait before requeueing an instance controller reconcile when progress is pending")
	// qps and burst
	...
	flag.IntVar(&dynamicClientPoolSize, "dynamic-client-pool-size", 1,
		"Number of independent HTTP connections for the dynamic client. Higher values reduce HTTP/2 stream contention.")
	...
	set, err := kroclient.NewSet(kroclient.Config{
		QPS:                   float32(qps),
		Burst:                 burst,
		DynamicClientPoolSize: dynamicClientPoolSize,
	})
	if err != nil {
	...
	rgd := resourcegraphdefinitionctrl.NewResourceGraphDefinitionReconciler(
		...
		instanceDefaultRequeueDuration,
		graph.RGDConfig{
