...
func (c *Coordinator[K]) addWatch(key K, req WatchRequest) error {
	...
	// If this nodeID is being re-declared with a different target than the
	// in-flight current entry, evict the stale current entry from the indexes
	// before installing the new one — unless the stale entry is shared with
	// the previous committed set (in which case the index still needs it).
