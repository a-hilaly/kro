...
func (n *Node) GetDesiredIdentity() ([]*unstructured.Unstructured, error) {
	...
	result, err := n.resolveWithOptions(true, identityPaths)
	if err == nil && n.Spec.Meta.Namespaced {
		inst := n.deps[graph.InstanceNodeID]
		normalizeNamespaces(result, inst.observed[0].GetNamespace())
	}
	return result, err
...
func (n *Node) resolveWithOptions(hard bool, pathsOnly []string) ([]*unstructured.Unstructured, error) {
	vars := n.templateVarsForPaths(pathsOnly)
	soft := !hard
	switch n.Spec.Meta.Type {
	case graph.NodeTypeInstance:
		return n.softResolve()
	case graph.NodeTypeCollection:
		return n.resolveCollection(vars, soft, len(pathsOnly) == 0)
	case graph.NodeTypeResource, graph.NodeTypeExternal:
		...
		return n.resolveSingle(vars, soft)
	default:
...
func (n *Node) resolveSingle(vars []*variable.ResourceField, continueOnPending bool) ([]*unstructured.Unstructured, error) {
	if n.Spec.Template == nil {
		return nil, nil
	}

	baseExprs, _ := n.exprSetsForVars(vars)
	values, _, err := n.evaluateExprsFiltered(baseExprs, continueOnPending)
	if err != nil {
	...
	result, err := n.resolveFieldsInTemplate(n.Spec.Template, values, vars)
	if err != nil {
...
func (n *Node) resolveCollection(vars []*variable.ResourceField, continueOnPending bool, setIndexLabel bool) ([]*unstructured.Unstructured, error) {
	baseExprs, iterExprs := n.exprSetsForVars(vars)
	baseValues, _, err := n.evaluateExprsFiltered(baseExprs, continueOnPending)
	if err != nil {
		if !IsDataPending(err) {
	...
	for idx, iterCtx := range items {
		...
		desired, err := n.resolveFieldsInTemplate(n.Spec.Template, values, vars)
		if err != nil {
			return nil, fmt.Errorf("node %q collection resolve: %w", n.Spec.Meta.ID, err)
		}
		if setIndexLabel {
...
func (n *Node) templateVarsForPaths(paths []string) []*variable.ResourceField {
	...
	for _, p := range paths {
		pathSet[normalizePath(p)] = struct{}{}
	}

	result := make([]*variable.ResourceField, 0, len(n.templateVars))
	for _, v := range n.templateVars {
		if _, ok := pathSet[normalizePath(v.Path)]; ok {
...
func (n *Node) resolveFieldsInTemplate(
