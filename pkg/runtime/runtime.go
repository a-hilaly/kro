...
//
// For collection resources, use GetCollectionResources to get the actual expanded
// resources. This method returns nil for resolved collections but still indicates
// ResourceStateResolved so dependency checks work correctly.
func (rt *ResourceGraphDefinitionRuntime) GetResource(id string) (*unstructured.Unstructured, ResourceState) {
	// Check if it's a collection that's been resolved
	resource, hasDescriptor := rt.resources[id]
	if hasDescriptor && resource.IsCollection() {
		if _, exists := rt.resolvedCollections[id]; exists {
			// Collection is resolved - return nil resource but resolved state
			// Caller should use GetCollectionResources for actual resources
			return nil, ResourceStateResolved
		}
	}

	// Did the user set the resource? (for non-collections)
...
func (rt *ResourceGraphDefinitionRuntime) evaluateStaticVariables() error {
	...
	// Evaluate static variables (skip iteration kind - they need iterator bindings)
...
// product of iterator bindings, then evaluates only the iteration kind
// expressions for each binding combination.
func (rt *ResourceGraphDefinitionRuntime) ExpandCollection(resourceID string) ([]*unstructured.Unstructured, error) {
	...
	// Build resource IDs for CEL environment (schema + resolved resources + resolved collections)
	...
	bindings := cartesianProduct(iterators, iteratorValues)

	// Check collection size limit to prevent resource exhaustion
	if len(bindings) > DefaultMaxCollectionSize {
		return nil, fmt.Errorf("collection %s would generate %d resources, exceeding the maximum limit of %d; consider reducing the forEach input size or splitting into multiple collections",
			resourceID, len(bindings), DefaultMaxCollectionSize)
	}

	// For each binding combination, create a resolved resource
	...
	expandedResources := make([]*unstructured.Unstructured, 0, len(bindings))
	for _, binding := range bindings {
		// Deep copy the original template
		template := resource.Unstructured().DeepCopy()

		// Resolve the template with the iterator bindings
		resolved, err := rt.resolveTemplateWithBindings(resourceID, template, binding)
		if err != nil {
...
func cartesianProduct(iterators []ForEachDimensionInfo, values [][]interface{}) []map[string]interface{} {
	...
	for {
		// Create binding for current combination
		binding := make(map[string]interface{})
		for i, iter := range iterators {
			binding[iter.Name] = values[i][indices[i]]
		}
...
// resolveTemplateWithBindings resolves iteration kind variables in a template
// with iterator variable bindings. Static and dynamic variables are already
// resolved in the template.
func (rt *ResourceGraphDefinitionRuntime) resolveTemplateWithBindings(
	resourceID string,
	...
	// Get iterator variable names from bindings
	iteratorNames := make([]string, 0, len(bindings))
	...
	// Build eval context with schema, resolved resources, resolved collections, AND iterator bindings
	evalContext := map[string]interface{}{
		"schema": rt.instance.Unstructured().Object,
	}
	for id, res := range rt.resolvedResources {
...
// Returns nil if the collection hasn't been expanded yet.
func (rt *ResourceGraphDefinitionRuntime) GetCollectionResources(resourceID string) []*unstructured.Unstructured {
	return rt.resolvedCollections[resourceID]
}
...
func (rt *ResourceGraphDefinitionRuntime) IsCollectionReady(resourceID string) (bool, string, error) {
	...
	// Per-item evaluation with `each` binding (AND semantics)
	for i, item := range items {
		context := map[string]interface{}{
			"each": item.Object,
		}
