...
var (
	listsTotal = prometheus.NewCounterVec(prometheus.CounterOpts{
		...
		Help: "Total number of API lists performed by reflectors.",
	}, []string{"name"})

	listsDuration = prometheus.NewSummaryVec(prometheus.SummaryOpts{
		Name: "reflector_list_duration_seconds",
		Help: "Duration of API list operations in seconds.",
	}, []string{"name"})
...
func init() {
	...
	// controller-runtime does not register a reflector metrics provider,
	// so this SetReflectorMetricsProvider call is not yet consumed.
