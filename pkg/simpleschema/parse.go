...
// parseSpec parses a spec into a Type and optional Markers.
// Handles both type strings and nested object maps.
func parseSpec(spec interface{}) (types.Type, []*Marker, error) {
	...
	case map[string]interface{}:
		...
		for name, field := range val {
			t, _, err := parseSpec(field)
			if err != nil {
				return nil, nil, fmt.Errorf("field %s: %w", name, err)
			}
			fields[name] = t
		}
		return types.Struct{Fields: fields}, nil, nil
	default:
