...
// TestCRDAddSeedsHashAndEnqueuesMatchingGraphs pins the CRD-add path: the
// hash is seeded, and matching subscribed Graphs (static + dynamic) are
// enqueued without broadcasting to unrelated Graphs.
func TestCRDAddSeedsHashAndEnqueuesMatchingGraphs(t *testing.T) {
	...
	// Only g1 (subscriber) and g3 (dynamic) enqueued; g2 is not enqueued.
	enq := drainEvents(sw, 100*time.Millisecond)
	assert.ElementsMatch(t, []client.ObjectKey{g1, g3}, enq)
	// Schema invalidator received the GK.
	assert.ElementsMatch(t, []schema.GroupKind{gkWidget}, si.snapshot())
	// Graph invalidator received only affected keys.
	assert.ElementsMatch(t, []client.ObjectKey{g1, g3}, gi.snapshot())
}
...
func TestHashCRDSchemaSelectivity(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			mutate: func(c *apiextensionsv1.CustomResourceDefinition) {
				c.Spec.Versions[0].Schema.OpenAPIV3Schema.Description = "v1"
			},
			sameHash: false,
		},
		{
...
// TestCRDAddRecovery_GraphTrackedBeforeCRDExists proves the recovery path:
// A Graph referencing a missing CRD registers its schema dependency and commits.
// When the CRD is later created (onAdd), the SchemaWatcher notifies invalidators
// and enqueues the Graph so it can re-reconcile and compile successfully.
func TestCRDAddRecovery_GraphTrackedBeforeCRDExists(t *testing.T) {
	sw, gi, si := newTestWatcher(t)

	graphKey := client.ObjectKey{Namespace: "default", Name: "recovery-graph"}
	gk := schema.GroupKind{Group: "custom.example.com", Kind: "LateWidget"}

	// Graph spec declares dependency on LateWidget and commits subscription
	// before the CRD is created in the cluster.
	sub := sw.ForGraph(graphKey)
	sub.Track(gk)
	sub.Done(true)

	// Verify reverse index has graphKey for gk before CRD exists.
	require.ElementsMatch(t, []client.ObjectKey{graphKey}, sw.GraphsForGroupKind(gk))

	// CRD is created later.
	crd := makeCRD("custom.example.com", "LateWidget", "v1")
	sw.onAdd(crd)

	// SchemaWatcher must notify schema invalidator, graph invalidator, and enqueue the Graph.
	assert.ElementsMatch(t, []schema.GroupKind{gk}, si.snapshot())
	assert.ElementsMatch(t, []client.ObjectKey{graphKey}, gi.snapshot())
	enqueued := drainEvents(sw, 100*time.Millisecond)
	assert.ElementsMatch(t, []client.ObjectKey{graphKey}, enqueued)
}
