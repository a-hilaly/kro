...
type Schema struct {
	...
	// Validation is a list of CEL validation rules applied to instances of the generated CRD.
	// Each validation rule can reference instance fields using 'self'.
	// Use this to enforce business logic and constraints beyond basic type validation.
	Validation []Validation `json:"validation,omitempty"`
	// AdditionalPrinterColumns defines custom columns to display when using 'kubectl get'.
	// These columns are added to the generated CRD and allow you to show important
	// instance information at a glance. If specified, default columns are not included,
	// so you must explicitly add them if needed.
	// Example: Display the instance's status and endpoint when listing resources.
	//
	// +kubebuilder:validation:Optional
	AdditionalPrinterColumns []extv1.CustomResourceColumnDefinition `json:"additionalPrinterColumns,omitempty"`
}

// Validation represents a CEL validation rule that is applied to instances of the ResourceGraphDefinition.
type Validation struct {
	// Expression is a CEL expression that must evaluate to true for the validation to pass.
	// The expression can reference fields from the instance's spec using 'self'.
	// Example: "self.replicas >= 1 && self.replicas <= 10"
	Expression string `json:"expression,omitempty"`
	// Message is the error message displayed when the validation fails.
	// It should clearly describe what condition was violated.
	Message string `json:"message,omitempty"`
}

// ExternalRefMetadata defines the metadata for an external resource reference.
